...
namespace Porter2Stemmer
{
    ...
    {
        private static EnglishPorter2Stemmer _instance;

        public static EnglishPorter2Stemmer Instance
        {
            get
            {
                if (_instance == null)
                {
                    _instance = new EnglishPorter2Stemmer();
                }
        ...
        public char[] Vowels => _vowels;

        private readonly string[] _doubles =
            { "bb", "dd", "ff", "gg", "mm", "nn", "pp", "rr", "tt" };
        public string[] Doubles => _doubles;

        private readonly char[] _liEndings = "cdeghkmnrt".ToArray();
        public char[] LiEndings => _liEndings;
        ...
        public string Step1BRemoveLySuffixes(string word, int r1)
        {
            ...
            {
                string trunc = ReplaceSuffix(word, suffix);//word.Substring(0, word.Length - suffix.Length);
            ...
            foreach (KeyValuePair<string, string> suffix in suffixes)
            {
                if (word.EndsWith(suffix.Key))
                {
                    if (SuffixInR1(word, r1, suffix.Key)
                        && TryReplace(word, suffix.Key, suffix.Value, out string final))
                    {
                        return final;
                    }
                    return word;
